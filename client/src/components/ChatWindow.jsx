...
export default function ChatWindow({ conversation }) {
  ...
  const sendMessage = async (type = "text") => {
    const content =
      type === "offer" ? `Offer: ${formatPrice(Number(offerAmount))}` : input.trim();
    ...
    try {
      const payload = {
        content,
        type,
        offerAmount: type === "offer" ? Number(offerAmount) : undefined,
      };
      ...
      <div style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <div>
            ...
            <div style={styles.carLabel}>
              re: {car.year} {car.make} {car.model}
            </div>
          </div>
        </div>
        ...
        <div style={styles.headerRight}>
          ...
          {conversation.lastOffer && (
            <span style={styles.offerTag}>
              Last offer: {formatPrice(conversation.lastOffer)}
            </span>
          )}

          {isBuyer && (
            <button
              onClick={() => navigate(`/cars/${car._id}#checkout`)}
              ...
              >
                ...
                >
                  ...
                  <div
                    style={{
                      ...styles.msgTime,
                      color: isMine ? "rgba(255, 254, 254, 0.57)" : "rgba(255,255,255,0.3)",
                    }}
                  >
        ...
        {offerMode ? (
          <div style={styles.offerRow}>
            ...
            <input
              ...
              autoFocus
            />
            <button onClick={() => sendMessage("offer")} style={styles.sendBtn}>
              Send offer
            </button>
            <button onClick={() => setOfferMode(false)} style={styles.cancelBtn}>
              Cancel
            </button>
          </div>
        ) : (
          <div style={styles.textRow}>
            <button onClick={() => setOfferMode(true)} style={styles.offerBtn} title="Make an offer">
              Offer
            </button>
            <textarea
              value={input}
...
const styles = {
  ...
  checkoutBtn: { background: "#22c55e", color: "#052e16", border: "none", borderRadius: 8, padding: "9px 12px", fontSize: 12, fontWeight: 800, cursor: "pointer" },
  ...
  offerBtn: { background: "transparent", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: "8px 10px", cursor: "pointer", fontSize: 14, color: "#f0f0ee" },
  textarea: {
    flex: 1,
    background: "#1c1f2a",
